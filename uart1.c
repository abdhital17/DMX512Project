...
void initUart0()
{
    ...
    UART0_IBRD_R = 10;                                  // r = 40 MHz / (Nx250kHz), set floor(r)=10, where N=16
    UART0_FBRD_R = 0;                                  // round(fract(r)*64)=0
    UART0_LCRH_R = UART_LCRH_WLEN_8 | UART_LCRH_STP2 | UART_LCRH_FEN;
...
{
    while (UART0_FR_R & UART_FR_TXFF);                  // wait if uart1 tx fifo full
