...
void displayUart0(char str[])
{
    ...
    while(str[i] != 0)
    {
        TXBuffer[writeIndex] = str[i];
        writeIndex = (writeIndex + 1) % BUFFER_LENGTH;
        i++;

    }

    if((UART0_FR_R & UART_FR_TXFE)  && i != 0)         //if TX fifo is empty and input string is not empty
    {
       UART0_DR_R = TXBuffer[readIndex];              // write character to fifo
       readIndex = (readIndex + 1) % BUFFER_LENGTH;
    }

    if (readIndex != writeIndex)
        UART0_IM_R |= 0x20;                       //enable the TX interrupt
}
...
void uart0ISR()
{
    ...
    if(full)
    {
        ...
        UART0_IM_R  &= ~0x20;                 //disable the TX interrupt
    }
